...
@section('title', 'Oyun Alanı Görsel Güncelleme')
<?php 
...
<!--begin::App Content Header-->
        <div class="app-content-header">
          ...
          <div class="container-fluid">
            <!--begin::Row-->
            <div class="row">
              <div class="col-sm-6"><h3 class="mb-0">Oyun Alanı Görsel Güncelleme</h3></div>
              <div class="col-sm-6">
                <ol class="breadcrumb float-sm-end">
                  <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">Anasayfa</a></li>
                  <li class="breadcrumb-item active" aria-current="page">Oyun Alanı Görsel Yönetimi</li>
                </ol>
              </div>
            </div>
            <!--end::Row-->
        ...
        <div class="app-content">
                ...
                <div class="card">
                    ...
                    ?>
                    <div class="card-body">
                        <form action="{{ route('admin.playground.images.store', $playground->id) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                                ...
                                <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}" id="tab-{{ $language->id }}" role="tabpanel" aria-labelledby="tab-{{ $language->id }}-tab">
                                    <div class="card-body" style="display:grid; grid-template-columns: 1fr 1fr 1fr; gap: 20px;">
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <input type="file" class="form-control" id="image_{{ $language->lang_code }}" name="image_{{ $language->lang_code }}" accept="image/*" value="{{ $photo[$language->lang_code] ?? '' }}">
                                            @if($photo[$language->lang_code])
                                                <img src="{{ $language->domain .'/'.  getFolder(['uploads_folder','playground_images_folder'], $language->lang_code) . '/' . $photo[$language->lang_code] }}" alt="{{ $alt[$language->lang_code] }}" style="width: 200px; height: auto; margin-top: 10px;">
                                                <input type="hidden" class="form-control" id="old_image_{{ $language->lang_code }}" name="old_image_{{ $language->lang_code }}" value="{{ $photo[$language->lang_code] }}" readonly>
                                            @endif
                                        </div>
                                        <div class="mb-3">
