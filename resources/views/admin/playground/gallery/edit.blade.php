...
@section('title', 'Oyun Alanı Galeri Görseli Güncelleme')
<?php 
...
<!--begin::App Content Header-->
        <div class="app-content-header">
          ...
          <div class="container-fluid">
            <!--begin::Row-->
            <div class="row">
              <div class="col-sm-6"><h3 class="mb-0">Oyun Alanı Galeri Görseli Güncelleme</h3></div>
              <div class="col-sm-6">
                <ol class="breadcrumb float-sm-end">
                  <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">Anasayfa</a></li>
                  <li class="breadcrumb-item active" aria-current="page">Oyun Alanı Galeri Yönetimi</li>
                </ol>
              </div>
            </div>
            <!--end::Row-->
                ...
                <div class="card">
                    ...
                    <?php 
                        foreach($gallery as $image){
                            $gallery_image[$image->lang] = $image->image;
                    ...
                    ?>
                    <div class="card-body">
                        <form action="{{ route('admin.playground.gallery.store', $playground->id) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                                ...
                                <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}" id="tab-{{ $language->id }}" role="tabpanel" aria-labelledby="tab-{{ $language->id }}-tab">
                                    <div class="card-body" style="display:grid; grid-template-columns: 1fr 1fr 1fr; gap: 20px;">
                                        ...
                                        <div class="mb-3">
                                            ...
                                            @if($gallery_image[$language->lang_code])
                                                <img src="{{ $language->domain .'/'.  getFolder(['uploads_folder','playground_images_folder'], $language->lang_code) . '/' . $gallery_image[$language->lang_code] }}" alt="{{ $alt[$language->lang_code] }}" style="width: 200px; height: auto; margin-top: 10px;">
                                                <input type="hidden" class="form-control" id="old_image_{{ $language->lang_code }}" name="old_image_{{ $language->lang_code }}" value="{{ $gallery_image[$language->lang_code] }}" readonly>
                                            @endif
                                        </div>
                                        <div class="mb-3">
